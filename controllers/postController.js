...
const getPost = async (req, res, next) => {
  ...
  if (!post) return next(new ApiError("The post not found", 404));
  res.status(200).json({
    status: "success",
    post,
  });
...
const getTimelinePosts = async (req, res, next) => {
  ...
  const followers = await Followers.find({ follower: req.user.id }).select(
    "user -_id",
  );
...
const getPostLikes = async (req, res, next) => {
  ...
  let likes = await Likes.find({ post: req.params.postId })
    .select("user -_id")
    .populate({
      path: "user",
