...
const getAllUsers = async (req, res) => {
  const users = await User.find({}, { password: 0, updatedAt: 0 }).sort({
    city: -1,
  });
  let userMap = [];
...
const getUser = async (req, res, next) => {
  ...
  const user = userId
    ? await User.findById(userId).select(
        "-password -updatedAt -createdAt -passwordChangedAt ",
      )
...
const searchUser = async (req, res) => {
  ...
  if (username) {
    const user = await User.find(query).select(
      "name profilePicture gender username _id",
    );
...
const updateUser = async (req, res, next) => {
  if (
    ...
    if (req.body.password) {
      ...
      return res
        .status(200)
        .json({ msg: "Account Has been Updated", updatedUser });
    } else {
...
const getUserFollowers = async (req, res, next) => {
  ...
  let followers = await Followers.find({ user: req.params.userId }).select(
    "follower -_id",
  );
...
const userInteraction = async (req, res, next) => {
  if (req.user._id !== req.params.id) {
    ...
    let follower;
    if (!isFollower) {
      follower = await Followers.create({
        user: req.params.id,
        follower: req.user._id,
      });
      res.status(200).json({
...
const unfollowUser = async (req, res) => {
  ...
  if (req.body.userId !== req.params.id) {
    ...
    if (user.followers.includes(req.body.userId)) {
      ...
      const followers = user.followers.filter((u) => {
        return u !== req.body.userId;
      });
      user.followers = followers;

      await currentUser.updateOne({ $pull: { followings: req.params.id } });

      const followings = currentUser.followings.filter((u) => {
        return u !== req.params.id;
      });
      currentUser.followings = followings;
      res
        .status(200)
        .json({ msg: "user Has Been Unfollowed", updatedUser: currentUser });
    } else {
